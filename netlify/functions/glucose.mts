...
export function normalizeReading(reading) {
  ...
  const [arrow, trend] = trendDisplay[reading.Trend] || ["?", "trend unavailable"];
  return {
...
export default async function handler(request) {
  ...
  try {
    const cacheKey = "__andrewGlucoseDisplayCache";
    const cache = globalThis[cacheKey];
    if (!cache || Date.now() - cache.cachedAt > 30000) {
      globalThis[cacheKey] = {
        ...(await fetchLatestReading(username, password, region)),
        cachedAt: Date.now()
      };
    }
    const { cachedAt, ...reading } = globalThis[cacheKey];
...
export const config: Config = { path: "/api/glucose" };
